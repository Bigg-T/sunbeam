...
let struct_tests = [
  t "s_test1" "define-struct document (author) 7" "7";
  t "s_test2" "define-struct document (rating) make-document (100)" "(struct 100)";
  terr "s_test3" "make-document (100)" "The structname document, used at <s_test3, 1:0-1:19>, was not defined";
  terr "s_test4" "define-struct document (rating) make-document (a)"
    "The identifier a, used at <s_test4, 1:47-1:48>, is not in scope";
  t "s_test5" "define-struct document (rating, isGood) make-document (100, true)" "(struct 100, true)";
  t "s_test6" "define-struct document (rating, isGood) let doc1 = make-document (100, true) in doc1" "(struct 100, true)";
  t "s_test7" "define-struct document (rating, isGood) let doc1 = make-document (100, true) in (document-rating doc1)" "100";
  t "s_test8" "define-struct document (rating, isGood) let doc1 = make-document (100, true) in (document-isGood doc1)" "true";
  t "s_test9" "define-struct document (rating, isGood)
              define-struct cabinet (d1, d2)
  ...
  t "s_test10" "define-struct document (rating, isGood)
               define-struct cabinet (d1, d2)
               let doc1 = make-document (100, true) in
                 let doc2 = make-document (401, false) in
                   let cab1 = make-cabinet (doc1, doc2) in
                     (document-rating (cabinet-d1 cab1))" "100";
  (* t "s_test11" "define-struct document (rating, isGood)
                let doc1 = make-document (100, true) in
                is document(doc1)"
                "true"; *)
  ...
  t "s_test11" "define-struct document (rating, isGood)
                ...
                document?(doc1)"
    "true";
  terr "s_test12" "define-struct document (rating, isGood)
                let doc1 = make-document (100, true) in
                dog?(doc1)"
    "The structname dog, used at <s_test12, 3:16-3:26>, was not defined";
  t "s_test13" "define-struct document (rating, isGood)
                let doc1 = make-document (100, true) in
                document?(5)"
    "false";
  t "s_test14" "define-struct document (rating, isGood)
              let doc1 = make-document (100, true) in
              begin
              ...
              " "false";
  t "s_test15" "define-struct document (rating, isGood)
              let doc1 = make-document (100, true) in
              begin
                (document-isGood doc1) := false;
...
The identifier doc1, used at <s_test16, 1:17-1:21>, is not in scope";
  terr "s_test17" "define-struct document (rating, isGood)
                  let doc1 = make-document (100, true) in
                  (document-author doc1)"
    "The struct document does not have field author used at <s_test17, 3:18-3:40>";
  terr "s_test18" "define-struct document (rating, isGood)
                  let doc1 = make-document (100, true) in
                  (document-author doc1) := 5"
    "The struct document does not have field author used at <s_test18, 3:18-3:45>";
  terr "s_test19" "define-struct document (rating, isGood)
                  let doc1 = make-document (100, true) in
                  (document-rating doc1) := (a + 5)"
    "The identifier a, used at <s_test19, 3:45-3:46>, is not in scope";
  terr "s_test20" "define-struct document (rating, isGood)
                    let doc1 = make-document (true) in
                    doc1"
    "The struct document expects 2 fields, received 1 at";
  terr "s_test21" "define-struct document (rating, isGood)
                define-struct document (grade)
                7"
    "The struct document used at <s_test21, 2:16-2:46> has already been defined";
  t "s_test22" "define-struct dog (isGood, id)
                define-struct cat (isGood, id)
                define-struct julie (dog, cat)
                let dog1 = make-dog (true, 1) in
                let cat1 = make-cat (true, 2) in
                let me = make-julie (dog1, cat1) in
                me"
    "(struct (struct true, 1), (struct true, 2))";
  t_opt "s_test23" "define-struct score (grade1, grade2)
                let score1 = make-score (70 + 3, 5) in
                score1"
     "(define-struct score (grade1, grade2))(alet binop_5 = 73 in (alet score1 = (make-struct score (binop_5, 5)) in score1))";
  t_opt "s_test24" "define-struct score (grade1, grade2)
                  let a = 70 + 3 in
                  let b = a + 73 in
                  let score1 = make-score (a + 73, 5) in
                  score1"
     "(define-struct score (grade1, grade2))(alet binop_7 = 146 in (alet score1 = (make-struct score (binop_7, 5)) in score1))";
  t_opt "s_test25" "define-struct score (grade1, grade2)
                 let score1 = make-score (73, 5) in
                 begin
                   (score-grade1 score1) := let a = 70 + 3 in let b = a + 73 in a + 73;
                   score1
                 end"
    "(define-struct score (grade1, grade2))(alet score1 = (make-struct score (73, 5)) in (alet binop_6 = 146 in (alet structget_3 = (score-grade1 score1) := binop_6 in (structget_3; score1))))";
  t_opt "s_test26" "define-struct score (grade1, grade2) 5" "5";
  t "s_test27" "define-struct grade (listofscores, scorefn)
                let grade1 = make-grade ((70, 80, 90), (lambda x, y, z: (x + y + z))) in
                let a = (grade-scorefn grade1) in
                a(5, 6, 7)"
    "18";
  t "s_test28" "define-struct dog (isGood)
                let dogs = ((make-dog (true)), (make-dog (true))) in
                dogs"
     "((struct true), (struct true))";
  t "s_test29" "define-struct dog (isGood)
...
let suite =
  "suite">:::tests @ mutable_tuple_tests @ pair_tests @ oom @ wfn_tests
             @ regression_tests @ gc @ letrec_tests @ combined_tests @ struct_tests
;;
